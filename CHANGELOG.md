...
- v1.3.2（已发行）- 2026.9.24
  v1.3.2 正式版！本版本接入 Windows 系统媒体控件（SMTC）进度条拖拽定位播放，LyricServer 协议升级至 v1.3.0 支持音量双向同步，并修复搜索框焦点与后台下拉残留问题，完善 CI 构建流程。

  新增：
  - SMTC 系统进度条拖拽定位：任务栏媒体弹窗与锁屏界面的播放进度条支持拖动定位（PlaybackPositionChangeRequested 回调转发至 MusicManager::seek），并定时同步播放位置时间轴，暂停时也刷新进度
  - LyricServer 协议升级至 v1.3.0：新增 volume 双向消息，客户端可下发音量调节播放器音量，服务端在音量变化时向所有客户端回推；新客户端连接即补推当前音量，保证多客户端音量条一致；协议文档同步更新

  修复：
  - 修复搜索框焦点逻辑：窗口从后台恢复时焦点自动还原不再触发搜索下拉自动弹出（改由点击搜索框触发）
  - 修复窗口隐藏到托盘后搜索下拉残留问题（窗口进入后台时自动收起搜索下拉）

  Build：
  - 修复 CI 构建时 py7zr 1.1.1+ 符号链接安全校验误判 Qt 官方 .7z 包导致下载失败的问题（固定 py7zr==1.1.0）

- v1.3.1（已发行）- 2026.8.29
  v1.3.1 正式版！修复了音乐库同步与中文路径相关的一批问题（均来自在线bug收集平台，感谢用户们的反馈！）。
  本版本上线完整的 GitHub Actions 自动打包流水线，实现每次提交自动产出安装包并提交回仓库，无需手动操作。欢迎体验.\setup下的最新开发版！
